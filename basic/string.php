<?php
    ...
    $text = "台灣近年來學生棒球「投球失憶」時有所聞，
    數量比起過去增加不少，
    投球失憶至今在全世界還找不到答案，
    台灣中華職棒（CPBL）富邦悍將總教練洪一中17日新莊球場賽前受訪，
    他不知道確切原因，
    他推測可能與重量訓練流行有關係。";
    ...
    // echo md5($number);
    // echo sha1($number);
    // echo md5(sha1($number));
    // echo sha1(md5($number));
    // echo md5(md5($number));
    // echo sha1(sha1($number));
    // echo crypt($number, "za");
    // echo md5(crypt($number, "za"));
